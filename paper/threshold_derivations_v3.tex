% =============================================================================
% §2 (v3)  Threshold Derivations for the Five-Step IC Validation Framework
% =============================================================================
% Version 3 — Daily Cross-Sectional Spearman Time-Series Framework
%
...
%
% Changelog v2\rightarrow v3:
%   - Fix #1 (BLOCKING): Corrected Step~3 Bonferroni threshold --- only
%     11/11 positive windows passes (not 10/11).  Added Holm--Bonferroni
%     verification.
%   - Fix #2 (FLAG): Renamed Kelly--Malamud P/T to ``IC Signal Fraction'';
%     the P/T bound operates at the return level, not the IC level, so the
%     citation has been heavily qualified.
%   - Fix #3 (FLAG): Recency ($\Delta>-0.02$) and decay ($\Delta_{\text{half}}>-0.01$)
%     thresholds in the migration table are now marked as empirically calibrated
%     (calibrated from Ridge pipeline~v2), pending formal derivation.
%   - Fix #4 (FLAG): Added explicit discussion of the conceptual shift from
%     \emph{per-stock} binomial (``are most stocks predictable?'') to
%     \emph{daily positive fraction} (``are most days predictable?'').
% To compile: tectonic threshold_derivations_v3.tex
% =============================================================================
...
\title{\textbf{Mathematical Derivations of Diagnostic Thresholds}\\
       \large Five-Step IC Validation Framework --- Revision 3}
\author{}
\date{}
\maketitle
...
\vspace{-8pt}
\begin{center}
\small\textbf{Key Revision (v3):} Corrected Bonferroni multiple-testing
threshold (Step~3 requires $11/11$ positive windows, not $10/11$);
renamed Kelly--Malamud $P/T$ to ``IC Signal Fraction'';
added conceptual-shift clarification for the daily positive fraction test;
marked preliminary thresholds as empirically calibrated.
...
\begin{remark}[Conceptual shift: stocks vs.\ days]
\label{rem:conceptual_shift}
The per-stock binomial test and the daily positive fraction test ask
different questions.  The per-stock test asks ``are most stocks
predictable?'' --- it counts how many individual equities yield a
positive rank IC when averaged across all available windows.  The daily
test asks ``are most days predictable?'' --- it counts the fraction of
trading days on which the cross-sectional rank correlation is positive.
These are complementary perspectives: a model may predict well on most
days (high $\hat{p}_{+}$) but fail on a minority of persistently
unpredictable stocks (low per-stock positive fraction).  We adopt the
daily framework because (a)~it naturally handles the dependence
structure among stocks, (b)~each daily IC aggregates information from
all $N$ stocks, yielding a larger effective sample size ($T$~vs.~$S$),
and (c)~it aligns with the standard error computations used throughout
this work.
\end{remark}
...
\subsection{IC Signal Fraction}

We define the \emph{IC Signal Fraction} as the proportion of cross-window
IC variation attributable to genuine predictive signal rather than
estimation noise:
\begin{equation}
    \text{IC-SF} = \frac{\sigma_{\text{sig}}^{2}}
                          {\sigma_{\text{total}}^{2}},
    \qquad
    \sigma_{\text{sig}}^{2} = s_{\IC}^{2}
                             - \frac{\sigma_{\IC}^{2}}{T_{\text{val}}},
    \qquad
    \sigma_{\text{total}}^{2} = s_{\IC}^{2}.
\end{equation}
Here $\sigma_{\text{sig}}^{2}$ is the genuine cross-window signal
variance (total window IC variance minus the estimation noise floor)
and $\sigma_{\text{total}}^{2}$ is the observed window-IC variance.

The IC Signal Fraction is an IC-level analog of the
Kelly--Malamud~\cite{kelly2013} $P/T$ ratio, which bounds the fraction
of \emph{return}-level variance that is predictable.  Unlike $P/T$, the
IC-SF operates on rank-correlation (Spearman~$\rho$) space and does not
require specifying a trading strategy to convert ICs into return-level
predictions.  It should be interpreted as a diagnostic for signal
stability, not as a direct estimate of predictable return variance.
...
\begin{align}
    ...
    \sigma_{\text{sig}}^{2} &= 0.0382^{2} - 9.91\times10^{-5}
                             = 0.001459 - 0.000099
                             = 0.001360, \\[4pt]
    \text{IC-SF} &= \frac{0.001360}{0.001459} = 0.932.
\end{align}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Step~3 binomial}: $p < 0.00455$ (previously $p < 0.01$).
          With $W=11$, only $11/11$ positive windows survives the correction
          ($P(X \geq 11) = 1/2048 \approx 0.00049$).  A result of $10/11$
          windows positive yields $P(X \geq 10) = 12/2048 \approx 0.00586$,
          which exceeds both the Bonferroni threshold $\alpha' = 0.00455$
          and the Holm--Bonferroni step-down threshold
          $\alpha'_{(3)} = 0.05/9 \approx 0.00556$.
...
\begin{table}[htbp]
\centering
\caption{Threshold migration from Revision~1 to Revision~3.}
\label{tab:migration}
\small
\begin{tabular}{llll}
\toprule
...
Step~3 binomial pass   & $p < 0.01$ & $p < 0.00455$ & Bonferroni correction; requires $11/11$ windows \\
Step~3 recency pass    & $\Delta > -0.03$ & $\Delta > -0.02$ & \S\ref{sec:thresh2}, $\approx 2\,\SE(\Delta^{(w)})$, prelim.\footnotemark \\
...
Step~5 decay pass      & $\Delta_{\text{half}} > -0.02$ & $\Delta_{\text{half}} > -0.01$ & Tighter, daily-IC calibrated; prelim.\footnotemark \\
Step~5 bootstrap pass  & $\text{CI}_{\text{lower}} > 0.02$ & $\text{CI}_{\text{lower}} > 0.016$ & \S\ref{sec:thresh1}, $\IC_{\min}$ \\
\bottomrule
\end{tabular}
\footnotetext{Recency and decay thresholds are empirically calibrated from
  Ridge pipeline~v2 results and pending formal derivation.  Recency:
  under $\Ho$, $\SE_{\text{per-window}} \approx 0.158/\sqrt{252} \approx 0.00995$,
  so a drift of $-0.02$ is $-2.01\,\SE$ (one-sided $p \approx 0.022$).
  Decay: the halving-time threshold $-0.01$ is conservatively set at
  roughly $1\,\SE$ of the gap estimator.
  These values are provided as provisional starting points;
  cross-validation against multiple model families is needed before
  operational use.}
\end{table}

\subsection{Validation Protocol}

After implementing Revision~3 thresholds, verify against the pipeline~v2
...
\begin{itemize}[nosep]
    ...
    \item Compare verdicts between Revision~1 and Revision~3 to ensure
...
\begin{table}[htbp]
\centering
\caption{Framework comparison: Revision~1 vs.\ Revision~3.}
\label{tab:comparison}
\begin{tabular}{p{3.5cm}p{3.8cm}p{3.8cm}}
\toprule
Aspect & Revision~1 & Revision~3 (this document) \\
\midrule
